...
(function registerConfiguration(): void {
	...
	registry.registerConfiguration({
		...
		'properties': {
			...
			'window.customTitleBarControls': {
				'type': 'boolean',
				'default': true,
				'scope': ConfigurationScope.APPLICATION,
				'markdownDescription': localize('window.customTitleBarControls', "Controls whether the Minimize, Maximize, and Close buttons should be shown in the custom title bar. This setting is effective only if {0} is set to `custom`.", '`#window.titleBarStyle#`')
			},
			'window.dialogStyle': {
				'type': 'string',
				'enum': ['native', 'custom'],
